...
for k=1:l
    if test_data_label(k)==up_label & predict_label(k)==up_label  
        TruePositive = TruePositive +1;
    end
    if test_data_label(k)==down_label & predict_label(k)==down_label 
        TrueNegative = TrueNegative +1;
    end 
    if test_data_label(k)==down_label & predict_label(k)==up_label  
        FalsePositive = FalsePositive +1;
    end

    if test_data_label(k)==up_label & predict_label(k)==down_label  
